...
Ещё сэмплер следит за скачками. Когда heap или RSS вырастает больше чем на 6 MiB от одного сэмпла к следующему, он пишет профиль heap тестового процесса в каталог пары, `<client>-heap-<t_ms>ms.pb.gz`, где `t_ms` идёт по часам сэмплов, и пишет файл в лог. Профиль heap в Go - это и его профиль allocs: `go tool pprof` показывает, что было занято на последнем GC, `-sample_index=alloc_space` - что выделено с начала процесса, а `-base` с более ранним профилем - что выделено между ними. Клиент пишет на паре не больше 8 профилей: heap без лимита памяти под массовой нагрузкой может скакать больше чем на 6 MiB каждые несколько секунд.
