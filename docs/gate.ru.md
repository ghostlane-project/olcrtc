...
- Ячеек WB Stream в списке не бывает: без токена WB они проваливаются из-за конфигурации, и такой провал должен оставаться блокирующим.

Unit-тесты держат список в этих правилах: каждый шаблон совпадает с ячейкой плана и называет провайдера, но не `wbstream`, у каждой записи есть URL issue и причина, и ни одна ячейка не совпадает с двумя записями.
