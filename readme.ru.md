...
| Failover вслед за переезжающим сервером | Supervisor перечитывает список профилей на каждом переходе (`Config.Reload`; CLI перечитывает файл конфига) и двигается по именам, так что клиент идёт по скользящему окну комнат: комната, добавленная при живой сессии, берётся в тот момент, когда эта сессия кончается, без перезапуска. Сервер, закрывший control-стрим намеренно (комнату вывели из оборота), или комната с исчерпанными handshake завершают сессию сразу, а не держат мёртвый туннель минуту. IPv6-литералы отклоняются локально, как только exit сказал, что маршрута IPv6 у него нет. |
| Список запасных комнат на мобильных | `AddFailoverRoom` / `ClearFailoverRooms` рядом с `SetRoom`: рантайм проходит основную комнату и запасные под supervisor'ом, один проход, перечитывая список на каждом переходе, - хост может дописывать комнаты в живое поколение; проход заканчивается ошибкой, и дальше работает собственный цикл повторов хоста. `SetSessionListener` называет комнату каждой открывшейся сессии - так хост, не читающий лог, отличает переезд от переподключения. |
