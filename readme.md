...
| Failover that follows a moving server | The supervisor re-reads its profile list at every advance (`Config.Reload`; the CLI re-reads the config file) and advances by name, so a client follows a rolling window of rooms: a room added while a session is live is used the moment that session ends, no restart. A server that closes the control stream on purpose - a retired room - or a room whose handshakes are exhausted ends the session at once instead of holding a dead tunnel for a minute. IPv6 literals are refused locally once the exit has said it has no IPv6 route. |
